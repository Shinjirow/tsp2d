...
public class Gene implements Comparable<Gene> {
    ...
    private class PMXConverter {
         ...
         */
        private java.util.Map<Integer, Integer> router = new HashMap<>();
    ...
    private Gene crossOver(int firstHalf, int secondHalf, Gene pair) {
        ...
        aChild.route = reviseRoute(aChild.route);
     ...
     *
     */
    private int[] reviseRoute(int[] route){
        int tmp;
        for (int i = 0; i < Map.yakusoku.length; i++) {
            if (route[Map.yakusoku[i][1]] != Map.yakusoku[i][0]) {
                tmp = route[Map.yakusoku[i][1]];
                for (int j = 0; j < route.length; j++) {
                    if (route[j] == Map.yakusoku[i][0]) {
                        route[j] = tmp;
                    }
                }
                route[Map.yakusoku[i][1]] = Map.yakusoku[i][0];
            }
        }
        return route;
    }
